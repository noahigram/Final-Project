...
(define-struct (recipe-book thing)
  ;; collection of recipes
  ;; shows what can be cooked with what
  (recipes)
  #:methods
...
(define-user-command (read book)
  "Shows a variety of different recipes. What items you can use to cook your food!")


;; new-recipe-book: (listof string) container -> recipe-book
;; Makes a recipe-book with the specified recipes
(define (new-recipe-book recipes location)
  (local [(define book
            (make-recipe-book '("recipe")
                              '()
                              location
                              recipes))]
...
(define (main-recipe-book location)
